...
async function addSubSection(req , res)
{
    try {
        ...
        const {title , description , timeDuration , section_id} = req.body;

        if(!title || !description || !timeDuration || !section_id)
        {
            return res.status(400).json(
                {
                    success:false,
                    message:'Rnter all the fields'
                }
            )
        }
...
async function updateSubSection(req , res)
{
    try {
        ...
        const {title , description , timeDuration , subSection_id} = req.body;

        if(!title || !description || !timeDuration || !subSection_id)
        {
            return res.status(400).json(
                {
                    success:false,
        ...
        const subSectionDeleted = await CourseSubSection.findById({subSection_id});
...
async function deleteSubSection(req , res){
    ...
    try {
        ...
        const subSectionDeleted = await CourseSubSection.findById({subSectionId});
        ...
        await CourseSubSection.findByIdAndDelete({section_id});
