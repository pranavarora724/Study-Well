...
function Sidebar() {
  ...
  const [newSidebarLinks, setNewSidebarLinks] = useState(sidebarLinks);
  ...
  function updateValues() {
    const newArray = [...newSidebarLinks];
    newArray[2] = {
      id: 3,
      name: "My Courses",
      path: `/dashboard/my-courses/${user._id}`,
      type: "Instructor",
      icon: "VscVm",
    };
    newArray[4] = {
      ...
      path: `/dashboard/enrolledCourses/${user._id}`,
      type: "Student",
      icon: "VscMortarBoard",
    };
    setNewSidebarLinks(newArray);
  }
  ...
  return (
    ...
    >
      <div className="bg-richblack-700  w-[220px] lg:w-[300px]  h-[100%] flex flex-col pt-10 md:pt-0">
        ...
        <div>
            {newSidebarLinks.map((element, index) => {
              return (
                <div key={index}>
                  {token ? (
                    <SidebarLink
            ...
            <div
              ...
              className="flex flex-row gap-x-4 p-4 text-white font-semibold cursor-pointer hover:bg-richblack-800 items-center"
            >
              <CgLogOut />
              <div>Logout</div>
            </div>
